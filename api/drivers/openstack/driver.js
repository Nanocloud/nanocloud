  ...
  createMachine(options) {
    return ConfigService.get(
      ...
      .then((config) => {
        ...
        net user ${config.openstackMachineUsername} ${password}
        Invoke-WebRequest ${config.plazaURI} -OutFile C:\\plaza.exe
        C:\\plaza.exe install
        rm C:\\plaza.exe
        Set-ExecutionPolicy RemoteSigned -force
        New-NetFirewallRule -Protocol TCP -LocalPort ${config.plazaPort} -Direction Inbound -Action Allow -DisplayName PLAZA`).toString('base64');
